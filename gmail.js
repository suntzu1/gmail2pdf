...
function AddToList(messageid, filename) {
  ...
  if (q) {
    ...
    messageQ.push({ m: messageid, f: [filename] });
  }
}
...
function ReadEmailTextAndAttachment(auth, message_id, markread) {
  ...
  gmail.users.messages.get({ auth: auth, userId: 'me', id: message_id },
    function (err, response) {
      ...
      if (message_raw) {
        base64topdf.base64Decode(message_raw, this.gmid + 'XXX.pdf');

        // Access the email body content, like this...
        data = message_raw;
        buff = new Buffer(data, 'base64');
        text = buff.toString();
        // // begin: using pdfkit and text/html to create pdf
        // doc = new pdfkit;
        // doc.pipe(fs.createWriteStream(this.gmid + '.pdf'));
        // doc.text(text);
        // doc.end();
        // // end: using pdfkit and text/html to create pdf
        var options = { format: 'Letter' };

        AddToList(this.gmid, this.gmid + '.pdf');
        hpdf.create(text, options).toFile(this.gmid + '.pdf', function (err, res) {
          if (err) return console.log(err);
          var start = res.filename.lastIndexOf('\\');
          var end = res.filename.indexOf('.pdf');
          var msid = res.filename.substring(start + 1, end)
...
function mergeFiles() {
  debugger;
  // const fls = messageQ.find(f => f.m === message_id);
  for (var x = 0; x < messageQ.length; ++x) {
    msgid = messageQ[x].m;
    fls = messageQ[x].f;
    pdfmerge(fls, { output: 'output/full_' + msgid + '.pdf' })
      .then((buffer) => {
        debugger;
      });
  }
